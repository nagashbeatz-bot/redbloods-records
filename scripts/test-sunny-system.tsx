...
const OPS: OperationsRaw = {
  ...
  clipItems: sec([{ projectId: P(1), category: "תאורה", amount: 1500, currency: "₪", status: "תכנון בלבד", hasTransaction: false }, { projectId: P(1), category: "ציוד", amount: 200, currency: "$", status: "תכנון בלבד", hasTransaction: false }, { projectId: P(1), category: "בוטל", amount: 999, currency: "₪", status: "בוטל", hasTransaction: false }]),
...
async function main() {
  ...
  check("clip_planning: cancelled rows excluded, totals per currency (no FX), planning-only", [cp.items.length, cp.summary[0].value, cp.items.every((i) => i.fields.planningOnly === true)], [2, { "₪": 1500, "$": 200 }, true]);
  const mt = q("meetings");
