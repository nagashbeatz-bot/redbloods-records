...
async function main() {
  ...
  console.log("Isolation / static (21, 29-32)");
  {
    ...
    check("no route / UI / cron / agent imports the primitives", [...walk(path.join(ROOT, "app")), ...walk(path.join(ROOT, "components")), ...walk(path.join(ROOT, "lib"))].filter((f) => /\.(ts|tsx)$/.test(f) && !f.includes(`${path.sep}partner${path.sep}actions${path.sep}`) && /partner\/actions\/(action-service|event-store|live|service)/.test(fs.readFileSync(f, "utf8"))).map((f) => path.relative(ROOT, f)), []);
