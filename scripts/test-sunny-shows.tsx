...
function main() {
  ...
  ok("baseline -9 + SHOWS / LABEL_DJ entries; depth DEEP_BRAIN_V1; Steven / Red Films still pending", ((v) => v.date > "2026.09.25" || (v.date === "2026.09.25" && v.n >= 9))({ date: SYSTEM_BASELINE_VERSION.split("-")[0], n: Number(SYSTEM_BASELINE_VERSION.split("-")[1]) }) && ["SHOWS", "LABEL_DJ"].every((d) => CAPABILITY_CHANGES.some((x) => x.version === "2026.09.25-9" && x.domain === d) && DOMAIN_KNOWLEDGE_DEPTH[d] === "DEEP_BRAIN_V1") && ["STEVEN", "RED_FILMS"].every((d) => DOMAIN_KNOWLEDGE_DEPTH[d] === "PENDING_DEEP_MISSION"));
