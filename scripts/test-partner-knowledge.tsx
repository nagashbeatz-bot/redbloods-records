...
void (async () => {
  ...
  console.log("\n9. static guards");
  {
    ...
    ok("11. no dynamic import / eval / Function / require in the knowledge layer or MCP adapter", !/import\(|\beval\(|new Function|require\(/.test(kn + dir("lib/integrations/partner-mcp").replace(/await import\("@\/lib\/(supabase|partner\/gateway\/server|partner\/bridge\/server|partner\/owner-knowledge\/server|partner\/act\/remote|supabase-server|roles)"\)/g, "")));
    ...
    ok("MCP audit tool type = the 3 read tools + the P1 answer tool + the P2 knowledge tool + the 5 act tools (matches the applied audit CHECK, Wave 0 migration 6d82d93a; partner_query stays NULL)", /tool: "partner_brief" \| "partner_resolve" \| "partner_entity" \| "partner_answer_question" \| "partner_propose_knowledge" \| "partner_plan_action" \| "partner_preview_action" \| "partner_approve_action" \| "partner_execute_plan" \| "partner_plan_status" \| null;/.test(fs.readFileSync(path.join(root, "lib/integrations/partner-mcp/store.ts"), "utf8")));
