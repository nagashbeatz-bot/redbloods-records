...
void (async () => {
  ...
  console.log("\n9. static guards");
  {
    ...
    ok("11. no dynamic import / eval / Function / require in the knowledge layer or MCP adapter", !/import\(|\beval\(|new Function|require\(/.test(kn + dir("lib/integrations/partner-mcp").replace(/await import\("@\/lib\/(supabase|partner\/gateway\/server|supabase-server|roles)"\)/g, "")));
    ...
    ok("MCP audit type still allows only the three CHECK-listed tool names", /tool: "partner_brief" \| "partner_resolve" \| "partner_entity" \| null;/.test(fs.readFileSync(path.join(root, "lib/integrations/partner-mcp/store.ts"), "utf8")));
