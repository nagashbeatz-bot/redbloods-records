...
async function main() {
  ...
  console.log("27-32. Append/read only; server-only; no route; no browser; no AI");
  {
    ...
    check("30. /api/partner holds only the actions surface + the three Owner routes (decide / change-deadline / execute) + the F.1M outcomes GET + the F2 finance GET + the F2.8 finance answer POST + the integrity GET / answer POST + the unified knowledge GET + the internal calendar read (service secret)", partnerRoutes, [path.join("actions", "change-deadline", "route.ts"), path.join("actions", "decide", "route.ts"), path.join("actions", "execute", "route.ts"), path.join("actions", "route.ts"), path.join("finance", "answer", "route.ts"), path.join("finance", "route.ts"), path.join("integrity", "answer", "route.ts"), path.join("integrity", "route.ts"), path.join("internal", "calendar", "route.ts"), path.join("knowledge", "route.ts"), path.join("outcomes", "route.ts")].sort()); // fbc81d3: + the secret-protected internal calendar read
