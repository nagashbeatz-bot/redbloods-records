...
async function main() {
  ...
  console.log("27-32. Append/read only; server-only; no route; no browser; no AI");
  {
    ...
    const intCore = fs.readFileSync(path.join(ROOT, INTEGRITY_CONTEXT_FILES[1]), "utf8").replace(/\/\*[\s\S]*?\*\//g, "");
    ok("30. integrity answer core never binds a store (injected append only)", !/context-store|createOwnerContextStore|lib\/supabase|server-only/.test(intCore) && /from "\.\.\/investigation\/context-persistence";/.test(intCore));
    ...
    ok("30. F2.8: the finance answer binding is server-only and appends only via the injected core after requireOwner", /^import "server-only";/m.test(finSvc) && /^import \{ appendOwnerContext \} from "\.\.\/investigation\/context-store";$/m.test(finSvc) && (finSvc.match(/appendOwnerContext/g) ?? []).length === 2 && /export async function answerFinanceQuestion[\s\S]*?requireOwner\(\)[\s\S]*?answerFinanceQuestionCore\(/.test(finSvc));
    const finCore = fs.readFileSync(path.join(ROOT, FINANCE_CONTEXT_FILES[2]), "utf8").replace(/\/\*[\s\S]*?\*\//g, "");
    ok("30. F2.8: the finance answer core never binds a store (injected append only)", !/context-store|createOwnerContextStore|lib\/supabase|server-only/.test(finCore) && /from "\.\.\/investigation\/context-persistence";/.test(finCore));
    ...
    check("30. /api/partner holds only the actions surface + the three Owner routes (decide / change-deadline / execute) + the F.1M outcomes GET + the F2 finance GET + the F2.8 finance answer POST + the integrity GET / answer POST + the unified knowledge GET + the internal calendar read (service secret) + the Wave 1 internal action relay (service secret, off by default)", partnerRoutes, [path.join("actions", "change-deadline", "route.ts"), path.join("actions", "decide", "route.ts"), path.join("actions", "execute", "route.ts"), path.join("actions", "route.ts"), path.join("finance", "answer", "route.ts"), path.join("finance", "route.ts"), path.join("integrity", "answer", "route.ts"), path.join("integrity", "route.ts"), path.join("internal", "act", "route.ts"), path.join("internal", "calendar", "route.ts"), path.join("knowledge", "route.ts"), path.join("outcomes", "route.ts")].sort()); // fbc81d3: + the secret-protected internal calendar read
    const knowledgeRoute = fs.readFileSync(path.join(partnerApi, "knowledge", "route.ts"), "utf8");
    ok("30. unified knowledge route is GET-only, requireOwner, imports only the Gateway read binding, never touches Owner Context", /export async function GET\(/.test(knowledgeRoute) && !/export (async )?function (POST|PUT|PATCH|DELETE)/.test(knowledgeRoute) && /requireOwner\(\)/.test(knowledgeRoute) && !/context-store|context-persistence|appendOwnerContext/.test(knowledgeRoute) && JSON.stringify([...knowledgeRoute.matchAll(/^import .* from "([^"]+)";$/gm)].map((m) => m[1]).sort()) === JSON.stringify(["@/lib/partner/gateway/server", "@/lib/require-auth", "next/server"]));
    const intAnswerRoute = fs.readFileSync(path.join(partnerApi, "integrity", "answer", "route.ts"), "utf8").replace(/\/\*[\s\S]*?\*\/|\/\/.*$/gm, "");
    ok("30. integrity answer route is POST-only, same-origin guarded, imports only the integrity binding + request guard, never touches the store directly", /export async function POST\(/.test(intAnswerRoute) && !/export (async )?function (GET|PUT|PATCH|DELETE)/.test(intAnswerRoute) && /const guard = checkSameOriginJson\(req\.headers\);\s*if \(guard\)/.test(intAnswerRoute) && !/context-store|context-persistence|appendOwnerContext/.test(intAnswerRoute) && JSON.stringify([...intAnswerRoute.matchAll(/^import .* from "([^"]+)";$/gm)].map((m) => m[1]).sort()) === JSON.stringify(["@/lib/partner/actions/request-guard", "@/lib/partner/integrity/server", "next/server"]));
    const intRoute = fs.readFileSync(path.join(partnerApi, "integrity", "route.ts"), "utf8");
    ok("30. integrity surface route is GET-only, imports only the integrity binding, never touches Owner Context", /export async function GET\(/.test(intRoute) && !/export (async )?function (POST|PUT|PATCH|DELETE)/.test(intRoute) && !/context-store|context-persistence|appendOwnerContext/.test(intRoute) && JSON.stringify([...intRoute.matchAll(/^import .* from "([^"]+)";$/gm)].map((m) => m[1]).sort()) === JSON.stringify(["@/lib/partner/integrity/server", "next/server"]));
    const finAnswerRoute = fs.readFileSync(path.join(partnerApi, "finance", "answer", "route.ts"), "utf8").replace(/\/\*[\s\S]*?\*\/|\/\/.*$/gm, "");
    ok("30. F2.8 finance answer route is POST-only, same-origin guarded, imports only the answer binding + request guard, never touches the store directly", /export async function POST\(/.test(finAnswerRoute) && !/export (async )?function (GET|PUT|PATCH|DELETE)/.test(finAnswerRoute) && /const guard = checkSameOriginJson\(req\.headers\);\s*if \(guard\)/.test(finAnswerRoute) && !/context-store|context-persistence|appendOwnerContext/.test(finAnswerRoute) && JSON.stringify([...finAnswerRoute.matchAll(/^import .* from "([^"]+)";$/gm)].map((m) => m[1]).sort()) === JSON.stringify(["@/lib/partner/actions/request-guard", "@/lib/partner/finance/answer-service", "next/server"]));
