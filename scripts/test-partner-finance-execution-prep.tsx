...
async function main() {
  ...
  console.log("Static write boundary (Part O)");
  {
    ...
    check("no app file names the finance execution RPC", SRC.filter((f) => /partner_execute_record_paid_expense/.test(fs.readFileSync(f, "utf8"))).map(rel), []);
    ...
    ok("the store can still only append deadline decisions and call the deadline RPC", /action_type: "UPDATE_PROJECT_DEADLINE";/.test(rd("lib/partner/actions/events.ts")) && /export const EXECUTE_RPC = "partner_execute_update_project_deadline";/.test(rd("lib/partner/actions/events.ts")) && !/mapFinanceActionEventRow\(res\.data\)/.test(rd("lib/partner/actions/event-persistence.ts")));
    check("RECORD_PAID_EXPENSE appears in no route and in no UI control — only display + read modules", SRC.filter((f) => /RECORD_PAID_EXPENSE/.test(strip(fs.readFileSync(f, "utf8")))).map(rel).sort(),
      // display card, the read parser, the DTO, the (non-executable) readiness registry, memory + its knowledge-contract text
      ["components/partner/PartnerOutcomeCard.tsx", "lib/partner/actions/finance-events.ts", "lib/partner/actions/outcome-dto.ts", "lib/partner/finance/actions.ts", "lib/partner/memory/contract.ts", "lib/partner/memory/core.ts"].sort());
    ok("the finance registry is still non-executable (EXECUTOR_BLOCKED) for every type", Object.values(FINANCE_ACTION_REGISTRY).every((c) => c.executable === false));
    ok("no finance approval / execute endpoint exists", !fs.existsSync(path.join(ROOT, "app/api/partner/finance/execute")) && !fs.existsSync(path.join(ROOT, "app/api/partner/finance/approve")) && !/RECORD_PAID_EXPENSE|finance/i.test(strip(rd("app/api/partner/actions/execute/route.ts"))));
