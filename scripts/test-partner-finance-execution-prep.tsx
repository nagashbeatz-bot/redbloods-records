...
async function main() {
  ...
  console.log("Static write boundary (Part O)");
  {
    ...
    check("RECORD_PAID_EXPENSE appears in no route — only display, read, surface and the narrow finance core", SRC.filter((f) => /RECORD_PAID_EXPENSE/.test(strip(fs.readFileSync(f, "utf8")))).map(rel).sort(),
      ["components/partner/PartnerActionCard.tsx", "components/partner/PartnerOutcomeCard.tsx", "lib/partner/actions/finance-events.ts", "lib/partner/actions/outcome-dto.ts", "lib/partner/actions/surface-dto.ts", "lib/partner/actions/surface.ts", "lib/partner/finance/actions.ts", "lib/partner/gateway/entity-common.ts", "lib/partner/memory/contract.ts", "lib/partner/memory/core.ts", "lib/partner/sunny/action-proposal.ts", "lib/partner/system/registry.ts"].sort()); // Gateway V1: a display label only (read-only); Sunny: a REFUSAL list only; system awareness: a description only
