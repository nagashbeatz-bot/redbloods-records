...
async function main() {
  ...
  console.log("Write safety + auth (10-13)");
  {
    ...
    ok("13. /api/partner is not a public bypass (the ONLY exceptions: the exact self-authenticated internal calendar read + the Wave 1 internal action relay, each with its own service secret)", (() => { const px = fs.readFileSync(path.join(ROOT, "proxy.ts"), "utf8"); const hits = (px.match(/"\/api\/partner[^"]*"/g) ?? []).sort(); return hits.length === 2 && hits[0] === "\"/api/partner/internal/act\"" && hits[1] === "\"/api/partner/internal/calendar\""; })());
