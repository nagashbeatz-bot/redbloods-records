...
async function main() {
  ...
  console.log("Write safety + auth (10-13)");
  {
    ...
    ok("13. /api/partner is not a public bypass (the ONLY exception: the exact self-authenticated internal calendar read)", (() => { const px = fs.readFileSync(path.join(ROOT, "proxy.ts"), "utf8"); const hits = px.match(/"\/api\/partner[^"]*"/g) ?? []; return hits.length === 1 && hits[0] === "\"/api/partner/internal/calendar\""; })());
