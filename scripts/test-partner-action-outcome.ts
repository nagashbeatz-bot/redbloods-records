...
async function main() {
  ...
  console.log("Static boundaries (14-20, 23)");
  {
    ...
    check("F.1M: the Outcome is consumed ONLY by its read-only chain (DTO, recent-outcomes core, server binding), the Owner-only GET route, the read-only Memory V1, the pure finance Outcome core and (Gateway V1) the read-only Gateway read context — no cron / agent / write path", importers.sort(), ["app/api/partner/outcomes/route.ts", "lib/partner/actions/finance-outcome.ts", "lib/partner/actions/outcome-dto.ts", "lib/partner/actions/outcome-server.ts", "lib/partner/actions/recent-outcomes.ts", "lib/partner/gateway/read-context.ts", "lib/partner/memory/core.ts", "lib/partner/memory/server.ts"]);
