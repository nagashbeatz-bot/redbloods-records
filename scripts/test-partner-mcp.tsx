...
async function main() {
  ...
  console.log("Static guards — read-only, business/dev separation (20, 21, 37, 47, 48)");
  {
    ...
    // Universal Action Layer (Wave 1): mcp.ts also imports the PURE act input validator; the act relay is a dynamic import in server.ts, bound only when the act switch is on — the connector itself still holds no business writer.
    ok("D. the only business capability is the read-only Gateway (brief / resolve / entity) + the gated act RELAY (no writer in the connector)", /import\("@\/lib\/partner\/gateway\/server"\)/.test(strip(rd(`${dir}/server.ts`))) && !code.some(([f, s]) => f !== `${dir}/server.ts` && f !== `${dir}/tools.ts` && /partner\/(?!gateway|act\/mcp-tools")/.test(s.match(/from "[^"]+"/g)?.join(" ") ?? "")));
