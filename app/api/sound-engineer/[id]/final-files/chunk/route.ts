...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === "finish") {
      ...
      const fin = await finalizeFinalFile({ workId, target: resolved.target, fileName, finalPath: uploaded.path_display, fileSize: uploaded.size ?? (offset + buffer.length), token, batchId, uploader: "owner" });
