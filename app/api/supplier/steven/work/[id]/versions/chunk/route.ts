...
// (start → append_v2 → finish). Same result/naming as the single-shot route.
export const maxDuration = 300;

const FORBID = () => NextResponse.json({ ok: false, error: "forbidden" }, { status: 403 });

/**
 * POST /api/supplier/steven/work/[id]/versions/chunk?action=start|append|finish
 *
 * Chunked version upload — OWNER-scope enforced exactly like the single-shot
 * version route: requireStevenAccess + assertStevenOwnsWork (Steven can only ever
 * write to his OWN work; anything else 403s). Only the `finish` step names + commits
 * the file and inserts the mix_versions row; the response is sanitized for Steven.
 */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error("[supplier/steven/versions/chunk]", msg);
