...
// ─── Rehearsal Modal — schedule a rehearsal for this show as an INDEPENDENT
//     session via the existing /api/sessions (no new API, no DB/show_id). ─────────

function RehearsalModal({ show, onClose, onCreated }: {
  show: Show;
  onClose: () => void;
  onCreated?: () => void;
}) {
  ...
  return (
    <>
      ...
      <div style={{
        position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)",
        background: "#141414", border: "1px solid #2A2A2A", borderRadius: 14,
        width: 360, maxWidth: "92vw", maxHeight: "90vh", overflowY: "auto", zIndex: 600, padding: 24, direction: "rtl",
      }}>
...
export default function ShowDrawer({ show, clients, onClose, onUpdated, onDeleted, onClientAdded }: Props) {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: "20px 20px 0", borderBottom: "1px solid #1E1E1E", paddingBottom: 14 }}>
          ...
          <button onClick={() => setRehearsalModal(true)} style={{
            ...
            color: "#818CF8", cursor: "pointer", fontSize: 12.5, fontWeight: 700, fontFamily: "inherit",
          }}>🥁 קבע חזרה</button>
        </div>
      ...
      {rehearsalModal && (
        <RehearsalModal show={draft} onClose={() => setRehearsalModal(false)} />
      )}
    </>
  );
}
