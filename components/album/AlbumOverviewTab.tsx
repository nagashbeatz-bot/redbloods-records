...
const TRACK_PCT: Record<AlbumTrackStatus, number> = {
  "הושלם": 100, "במיקס": 75, "מחכה למיקס": 55, "בעבודה": 35, "בהשהייה": 10, "לא התחיל": 0, "בוטל": 0,
};
        ...
        <div style={card}>
          <div style={cardHead()}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <button style={addBtn("income")} onClick={() => setShowAddTx("income")}>+ הוסף הכנסה</button>
              <button style={addBtn("expense")} onClick={() => setShowAddTx("expense")}>+ הוסף הוצאה</button>
            </div>
          </div>
          ...
          {showAddTx && (
            <QuickTxModal
              ...
              onClose={() => { setShowAddTx(null); load(); }}
            />
          )}
        </div>
