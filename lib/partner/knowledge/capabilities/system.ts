...
import * as MX from "../../system/mix";
import * as RFC from "../../system/red-films";
...
export const systemAwareness: KnowledgeCapability = {
  ...
  modes: {
    ...
    red_films_model: { descriptionForModel: "RED FILMS + CLIP / VIDEO as Redbloods implements it (param section): fields (every production / budget line / Red Films payment / crew / document / equipment / reference / scene / clip planning field), settings (clip price + managed production), vocabularies, work (two systems, work unit, project type, send clip), statuses (production + edit status, lifecycle recorded vs not, consistency), money (planned → Red Films ledger → actual Finance expense → paid; promote; shoot expense; currency; clip deal; recoup), identity (crew = free text), calendar (shoot days, multiple shoots, completion, calendar link, preparation), files (storage, documents, footage, versions, delivery, publication), consumers, actions, workflows, signals, security, integrity" },
    mix_model: { descriptionForModel: "STEVEN + MIX PIPELINE as Redbloods implements it (param section): fields (every engineer-work / version / comment / attachment / riddim line / pre-mix note / final-file field), settings, vocabularies, identity (Steven vs other engineers), steven_vs_generic, work (unit, cardinality, creation paths, send log), statuses (stored vs Steven display, automatic moves, completion, reopen), handoff (production → mix, mix start, the evidence ball rule, conflicts, caveats), versions (rounds, latest, ordering, approval), feedback (comments: who writes / resolves, per version, pre-mix, attachments, notes send), files (final files), money (two finance writers, fixed ratio, expense scope, statuses, rate reality), portal, pushes, consumers (agent alerts, COO, cases, album tracks, tasks, calendar), actions, workflows, signals, security, integrity" },
  ...
  read(_src, q) {
    ...
    if (q.mode === "red_films_model") {
      const t = q.params.section ?? "work";
      ...
      const rowsR: Array<{ id: string; label: string; fields: Record<string, unknown> }> =
        ...
        : t === "identity" ? ent(RFC.CREW_MODEL)
        : t === "calendar" ? ent(RFC.SHOOT_MODEL)
        : t === "files" ? ent(RFC.FILES_MODEL)
        : t === "consumers" ? ent(RFC.OTHER_CONSUMERS)
        : t === "actions" ? RFC.RF_ACTIONS.map((a) => { const { internal: _i, ...served } = a; void _i; return { id: a.id, label: a.action, fields: { ...served } }; })
        : t === "workflows" ? RFC.RF_WORKFLOWS.map((w) => ({ id: w.event, label: w.concept, fields: { ...w } }))
        : t === "signals" ? RFC.RF_SIGNAL_MODEL.map((x) => ({ id: x.code, label: x.note, fields: { ...x } }))
        : t === "security" ? [obj("enforcement", "enforcement", RFC.SECURITY_REVIEW.enforcement), ...RFC.SECURITY_REVIEW.findings.map((f, i) => ({ id: `finding:${i}`, label: f, fields: {} }))]
        : t === "integrity" ? [obj("counts", "Production integrity counts (read-only, 2026-09-25)", RFC.RF_INTEGRITY.productionCounts20260925), ...RFC.RF_INTEGRITY.findingsHe.map((f, i) => ({ id: `finding:${i}`, label: f, fields: {} }))]
        : ent(RFC.DOMAIN_MODEL);
      return result(rowsR.map((r) => item({ id: r.id, label: partner(r.label), epistemic: "FACT", source: SRC, fields: r.fields })), { ...base, summary: [version(), sfact("RED_FILMS_BASELINE", "גרסת הידע על Red Films / קליפים", RFC.RED_FILMS_BASELINE_VERSION, "FACT", SRC)] });
    }
    if (q.mode === "mix_model") {
      const t = q.params.section ?? "handoff";
      const obj = (id: string, label: string, v: unknown) => ({ id, label, fields: (v && typeof v === "object" && !Array.isArray(v) ? { ...(v as Record<string, unknown>) } : { value: v }) });
      const ent = (o: Record<string, unknown>) => Object.entries(o).map(([k, v]) => obj(k, k, v));
      const rowsM: Array<{ id: string; label: string; fields: Record<string, unknown> }> =
        t === "fields" ? MX.MIX_FIELDS.map((f) => ({ id: `${f.entity}.${f.field}`, label: f.meaning, fields: { ...f } }))
        : t === "settings" ? MX.MIX_SETTINGS.map((x) => ({ id: x.key, label: x.meaning, fields: { ...x } }))
        : t === "vocabularies" ? ent(MX.MIX_VOCABULARIES)
        : t === "identity" ? ent(MX.IDENTITY_MODEL)
        : t === "steven_vs_generic" ? MX.STEVEN_VS_GENERIC.map((x, i) => ({ id: `scope:${i}`, label: x.behavior, fields: { ...x } }))
        : t === "work" ? ent(MX.WORK_MODEL)
        : t === "statuses" ? ent(MX.STATUS_MACHINE)
        : t === "versions" ? ent(MX.VERSION_MODEL)
        : t === "feedback" ? ent(MX.COMMENT_MODEL)
        : t === "files" ? ent(MX.FINAL_FILES_MODEL)
        : t === "money" ? ent(MX.MONEY_MODEL)
        : t === "portal" ? ent(MX.PORTAL_MODEL)
        : t === "pushes" ? MX.MIX_PUSHES.map((x) => ({ id: x.id, label: x.trigger, fields: { ...x, sunnyMaySend: false } }))
        : t === "consumers" ? ent(MX.OTHER_CONSUMERS)
        : t === "actions" ? MX.MIX_ACTIONS.map((a) => { const { internal: _i, ...served } = a; void _i; return { id: a.id, label: a.action, fields: { ...served } }; })
        : t === "workflows" ? MX.MIX_WORKFLOWS.map((w) => ({ id: w.event, label: w.concept, fields: { ...w } }))
        : t === "signals" ? MX.MIX_SIGNAL_MODEL.map((x) => ({ id: x.code, label: x.note, fields: { ...x } }))
        : t === "security" ? [obj("enforcement", "enforcement", MX.SECURITY_REVIEW.enforcement), ...MX.SECURITY_REVIEW.findings.map((f, i) => ({ id: `finding:${i}`, label: f, fields: {} }))]
        : t === "integrity" ? [obj("counts", "Production integrity counts (read-only, 2026-09-25)", MX.MIX_INTEGRITY.productionCounts20260925), ...MX.MIX_INTEGRITY.findingsHe.map((f, i) => ({ id: `finding:${i}`, label: f, fields: {} }))]
        : ent(MX.HANDOFF_MODEL);
      return result(rowsM.map((r) => item({ id: r.id, label: partner(r.label), epistemic: "FACT", source: SRC, fields: r.fields })), { ...base, summary: [version(), sfact("MIX_BASELINE", "גרסת הידע על מיקס / סטיבן", MX.MIX_BASELINE_VERSION, "FACT", SRC)] });
    }
