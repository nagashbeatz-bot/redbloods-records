...
export function buildCompanyView(src: GatewaySources) {
  ...
  for (const k of known) {
    ...
    // the same question asked live by the domain is folded into the known decision (one question, with live evidence)
    if (k.supersedes) for (let i = decisions.length - 1; i >= 0; i--) if (decisions[i].origin === "LIVE_QUESTION" && decisions[i].domain === k.supersedes.domain && k.supersedes.text.test(decisions[i].questionHe)) { st.e = `${st.e} · ${decisions[i].questionHe}`; decisions.splice(i, 1); }
