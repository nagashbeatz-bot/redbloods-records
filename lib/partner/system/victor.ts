...
export const VICTOR_FIELDS: readonly VictorField[] = [
  ...
  F("dropbox_folder", "POSSIBLE_BUG", "The work's storage folder. Victor can PATCH it to ANY path; uploads then land there (security gap).", { writers: "lazy folder creation on first upload; Owner; Victor (!)", victorSees: "no (stripped) — but writable", sunnyReads: "victor_view (path metadata)" }),
  F("dropbox_share_link", "CANONICAL", "PUBLIC share link to the work folder (bearer access material).", { victorSees: "no (stripped) — but writable", sunnyReads: "hasFolderLink only (secret)" }),
  ...
  F("files_sent", "CONFLICT", "EVERY version upload (Victor's AND the Owner's) lands here: {name, versionLabel, uploadedAt, path, share link, duration, size}. Victor can rewrite the whole array (security gap).", { writers: "upload routes; Owner / Victor PATCH; Victor file delete", victorSees: "sanitized (no path / links; opaque file ref)", sunnyReads: "victor_view (metadata; links → boolean)" }),
  F("files_received", "LEGACY", "Same shape — no code path writes it (always empty); writable by Victor's PATCH (security gap).", { writers: "nobody (Victor PATCH possible)" }),
...
export const FILE_MODEL = {
  uploads: "single upload or chunked (> 140 MB, 1 GB cap) into the work folder's Production subfolder; mode add + autorename (no overwrite); single uploads also create a PUBLIC share link per file",
  ...
  delete: "Victor: delete any file entry of any Victor work (incl. the Owner's) by file ref — storage delete first. Owner: delete by path then rewrite the list. No replace.",
  stream: "stream / download by work + file ref (owner or Victor); a legacy path form matches any Victor work's files; returns a temporary storage link",
...
export const PORTAL_MODEL = {
  ...
  victorCan: ["read his works (sanitized)", "upload versions", "stream / download", "delete ANY file entry of his works (incl. Owner uploads)", "PATCH file lists + folder fields (security gap)", "create folders + get public links via the vendor-folder route (security gap)", "change language / avatar", "ping presence, register push"],
  victorCannot: ["status / deadline / outcome", "notes", "salary / settings / notify routes"],
  privacyLeak: "the page payload includes salary + payment status (UI hides it)",
} as const;
...
export const VICTOR_REVIEWED_FINGERPRINTS: Readonly<Record<string, string>> = {
  "lib/vendor-store.ts": "8633d999b09969301c7947ed82c94e3df61b069b1c2dbf07afacefe41559d24c",
  "lib/vendor-folder.ts": "f1e0c75cf1c78932eefd5202717c9c4ad09f9e40b641665dca049508af50596a",
  ...
  "app/api/vendor/victor/work/route.ts": "ba6b729748e4070acd4936fdd2c90c7610c3a14b982c55591445c81e5c7b98a2",
  "app/api/vendor/victor/work/[id]/route.ts": "5b1a0cb5531f0a9a8e158342dbcea0889eda7ba6e5a482807b6aaeb05721e6f6",
  "app/api/vendor/victor/work/[id]/file/route.ts": "1357cfe511589ebb6807694627cb61a9a3938984822f051c2c04c02306b25f93",
  "app/api/vendor/victor/salary/route.ts": "f1f09e33afce9489d40a42b8e740dc5efbaf0be1db1b5c43dd3f0d8f1a23ff48",
  "app/api/dropbox/vendor-upload/route.ts": "3ea0b9d9bec6f797d21e9c2e35dc2fdf9031a40294f121985cc9ac115c68de57",
  "app/api/dropbox/vendor-upload/chunk/route.ts": "472bb7873a1686ccc5160cf18f4d7fb218d62dc9fd78a3479112c9127d523593",
  "app/api/dropbox/vendor-folder/route.ts": "bafe24a9d6c0c5aae8c08b1722fa771c95c3f1a7d8672c071f3af3345022406b",
};
