...
export const KNOWLEDGE_GAPS: readonly KnowledgeGap[] = [
  ...
  G({ id: "PRJ_CALENDAR_LIVE", domain: "GOOGLE_CALENDAR", entityType: "calendar event", class: "CAPABILITY_GAP", inherited92671ee: "Google Calendar beyond sessions not read",
    description: "Live Google Calendar content", redbloodsKnows: "which sessions / shows / meetings have an event (event id) and the Google Task ids of tasks; Google holds titles, attendees, moves",
    sunnyKnows: "every Redbloods-tracked calendar link of the project (sessions, meetings, tasks) with dates; NOT the live Google event", why: "every Google read may refresh the OAuth token and WRITE it back to settings — a credential write inside Sunny's read path is not allowed without Owner approval; project matching of untracked events is fuzzy text",
    wouldClose: "Owner-approved read-only calendar primitive that does not persist refreshed tokens (no new OAuth scope needed: calendar.readonly is already granted)", relationshipsAffected: ["project → calendar events"], risk: "LOW", ownerImpact: "manually created calendar events about a project are not visible", futureActionImpact: "calendar writes stay EXTERNAL_EFFECT", status: "PARTIALLY_CLOSED", appRemediation: "OAUTH_OR_INTEGRATION_APPROVAL", sunnyReadsVia: ["project_view:calendar"] }),
...
export const DOMAIN_KNOWLEDGE_DEPTH: Readonly<Record<string, DomainDepth>> = {
  ...
  CLIENTS: "PENDING_DEEP_MISSION", PROPOSALS: "PENDING_DEEP_MISSION", SESSIONS: "PENDING_DEEP_MISSION", GOOGLE_CALENDAR: "PENDING_DEEP_MISSION", TASKS: "PENDING_DEEP_MISSION",
