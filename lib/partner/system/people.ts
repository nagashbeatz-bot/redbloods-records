...
export const USER_CONTRACTS: readonly UserContract[] = [
  ...
  },
  {
    ...
    securityGapIds: ["SG_STEVEN_PAYMENT_WRONG_RECIPIENT", "SG_ENGINEER_ROUTES_ASSUME_STEVEN"],
...
export const SECURITY_GAPS: readonly SecurityGap[] = [
  ...
  G("SG_RED_FILMS_ROUTES_PROXY_ONLY", "LOW", "UI_SERVER_MISMATCH", ["OWNER"], "Found in the 2026-09-25 video re-audit: most Red Films routes (productions, budget lines / payments / receipts, documents, references, folder, bulk delete) and the clip planning routes (incl. 'העבר לכספים') have no in-route Owner check — the central gate is the only layer (no other role is allowed there)."),
  G("SG_ENGINEER_ROUTES_ASSUME_STEVEN", "LOW", "PRIVACY", ["STEVEN", "EXTERNAL_ENGINEERS"], "Found in the 2026-09-25 mix re-audit: the Owner-only 'Send notes' / 'Send to Steven' routes and the payment-expense route have no engineer check — used on another engineer's work they would push Steven about that work and convert its price at Steven's fixed ₪ ratio. Not exercised (no other engineer exists)."),
  ...
  G("SG_AGENT_CHECK_INFO_LEAK", "LOW", "SECURITY_GAP", ["SCHEDULED_CALLERS"], "With the agent-alert rules switched off, the agent check answers callers without the secret with small holiday counts."),
