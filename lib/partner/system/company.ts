...
export const DISCOVERIES: readonly Discovery[] = [
  ...
  { id: "LEGACY_AI_AGENT", what: "A second, older in-app AI assistant existed beside Sunny (own prompt, context builder, OpenAI / Groq router, AI budget log, memory store) — RETIRED and REMOVED on 2026-09-25 by Owner decision, including its storage (the empty memory table was dropped and its six AI budget / log settings keys deleted, with a rollback backup)", where: "removed", why: "one organizational brain: Sunny", domains: ["SUNNY_CORE", "AGENT_ALERTS"], sunnyReads: "system knowledge (history)", gap: "CO_LEGACY_AI_BRAIN", ownerInput: false },
