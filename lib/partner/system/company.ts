...
export const REPO_COVERAGE: ReadonlyArray<{ pattern: string; cls: ModuleClass; domain: string; note: string }> = [
  ...
  { pattern: "^lib/(agent/|agent-core|ai-router|openai|providers/|mai/|reports/)", cls: "LEGACY", domain: "AGENT_ALERTS / REPORTS", note: "the in-app 'מאי AI' agent (kill-switched), its alert rules, AI budget and the daily morning / evening report emails" },
...
export const TABLE_COVERAGE: Readonly<Record<string, string>> = {
  ...
  business_memory: "GAP (legacy in-app AI memory; 0 rows; not read)", clients: "client_view", clip_items: "video_view", final_files: "mix_view", label_artists: "artist_view", label_media_income: "artist_view", meetings: "client_view / project_view",
...
export const DISCOVERIES: readonly Discovery[] = [
  { id: "AGENT_ALERT_TYPES", what: "The agent alert engine has 16 alert types (overdue / due-soon deadlines, session needs update, stale session, payment overdue, balance missing due date, project without pricing, proposal follow-up due, Victor stuck / below pace, inactivity, monthly goals behind, completed without delivery, upcoming holiday, week understaffed) — a parallel attention engine; company-level alerts (goals, week, holiday) have no project and were invisible to Sunny", where: "agent rules + week-strength + holiday check; open alerts in production", why: "the Owner sees these alerts in the app; Sunny must not contradict or duplicate them", domains: ["AGENT_ALERTS", "COMPANY_OVERVIEW"], sunnyReads: "company_view attention (open company-level alerts)", gap: "CO_AGENT_ALERTS_PARALLEL", ownerInput: false },
  { id: "LEGACY_AI_AGENT", what: "A second, older in-app AI ('מאי AI'): its own system prompt / project context builder, OpenAI + Groq router, AI budget log, a memory table (0 rows) and alert rules — kill-switched in the UI", where: "agent-core, ai-router, providers, mai, business memory", why: "two AI brains with different memories and rules can disagree; its memory is not Sunny's P2", domains: ["AGENT_ALERTS", "SUNNY_CORE"], sunnyReads: "system knowledge only (the memory table is empty)", gap: "CO_LEGACY_AI_BRAIN", ownerInput: true },
