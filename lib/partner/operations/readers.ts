...
async function read(client: OperationsReadClient, table: string, columns: string, filter?: (q: OpsQuery) => OpsQuery): Promise<Section<Record<string, unknown>> | null> {
...
const map = <T,>(sec: Section<Record<string, unknown>> | null, f: (r: Record<string, unknown>) => T | null): Maybe<T> =>
...
export async function readOperationsRaw(client: OperationsReadClient): Promise<OperationsRaw> {
  const [prods, items, pays, equip, clip, meet, acts, beats, assign, camps, content, promos, cycles, tracks, work, versions, comments, finals, deliv, integ, pmeta] = await Promise.all([
    ...
    read(client, "red_films_equipment", "category, status"),
    read(client, "clip_items", "project_id, category, amount, currency, status, linked_transaction_id"),
    read(client, "meetings", "id, date, time, status, project_id, client_id, calendar_event_id"),
    read(client, "project_actions", "id, project_id, action_type, content_type, recipient_role, status, action_date, followup_date"),
    read(client, "beats", "id, name, genre, musical_key, status, created_at"),
    read(client, "beat_artist_assignments", "beat_id, artist_slug"),
    read(client, "social_campaigns", "id, project_id, title, artist_name, release_date, status, promotion_budget"),
    read(client, "social_content_items", "campaign_id, status, content_type, platform, due_date, publish_date"),
    read(client, "social_promotions", "campaign_id, channel, planned_amount, status, promo_date, linked_transaction_id"),
    read(client, "artist_balance_cycles", "artist_id, cycle_index, start_date, end_date, income, payments, expenses, ending_balance, closed_at"),
    read(client, "album_tracks", "project_id, track_number, title, status, mix_status, master_status"),
    read(client, "sound_engineer_work", "id, project_id, engineer_name, work_type, work_title, status, sent_date, internal_deadline, agreed_price, amount_paid, currency, payment_date"),
    read(client, "mix_versions", "id, sound_engineer_work_id, status, created_at"),
    read(client, "mix_comments", "mix_version_id, status"),
    read(client, "final_files", "work_id, created_at"),
  ...
  return {
    ...
    budgetItems: map(items, (r) => (s(r.production_id) ? { productionId: String(r.production_id), planned: n(r.planned_amount), actual: n(r.actual_amount), status: s(r.status), hasTransaction: !!s(r.linked_transaction_id) } : null)),
    budgetPayments: map(pays, (r) => (s(r.production_id) ? { productionId: String(r.production_id), amount: n(r.amount), paymentDate: s(r.payment_date) } : null)),
    equipment: map(equip, (r) => ({ category: s(r.category), status: s(r.status) })),
    clipItems: map(clip, (r) => ({ projectId: s(r.project_id), category: s(r.category), amount: n(r.amount), currency: s(r.currency), status: s(r.status), hasTransaction: !!s(r.linked_transaction_id) })),
    meetings: map(meet, (r) => (s(r.id) ? { id: String(r.id), date: s(r.date), time: s(r.time), status: s(r.status), projectId: s(r.project_id), clientId: s(r.client_id), hasCalendarEvent: !!s(r.calendar_event_id) } : null)),
    projectActions: map(acts, (r) => (s(r.id) ? { id: String(r.id), projectId: s(r.project_id), actionType: s(r.action_type), contentType: s(r.content_type), recipientRole: s(r.recipient_role), status: s(r.status), actionDate: s(r.action_date), followupDate: s(r.followup_date) } : null)),
    beats: map(beats, (r) => (s(r.id) ? { id: String(r.id), name: s(r.name) ?? "", genre: s(r.genre), musicalKey: s(r.musical_key), status: s(r.status), createdAt: s(r.created_at) } : null)),
    beatAssignments: map(assign, (r) => (s(r.beat_id) && s(r.artist_slug) ? { beatId: String(r.beat_id), artistSlug: String(r.artist_slug) } : null)),
    campaigns: map(camps, (r) => (s(r.id) ? { id: String(r.id), projectId: s(r.project_id), title: s(r.title) ?? "", artistName: s(r.artist_name), releaseDate: s(r.release_date), status: s(r.status), promotionBudget: n(r.promotion_budget) } : null)),
    contentItems: map(content, (r) => ({ campaignId: s(r.campaign_id), status: s(r.status), contentType: s(r.content_type), platform: s(r.platform), dueDate: s(r.due_date), publishDate: s(r.publish_date) })),
    promotions: map(promos, (r) => ({ campaignId: s(r.campaign_id), channel: s(r.channel), plannedAmount: n(r.planned_amount), status: s(r.status), promoDate: s(r.promo_date), hasTransaction: !!s(r.linked_transaction_id) })),
    balanceCycles: map(cycles, (r) => (s(r.artist_id) ? { artistId: String(r.artist_id), cycleIndex: Number(r.cycle_index), startDate: s(r.start_date), endDate: s(r.end_date), income: n(r.income), payments: n(r.payments), expenses: n(r.expenses), endingBalance: n(r.ending_balance), closedAt: s(r.closed_at) } : null)),
    albumTracks: map(tracks, (r) => ({ projectId: s(r.project_id), trackNumber: n(r.track_number), title: s(r.title) ?? "", status: s(r.status), mixStatus: s(r.mix_status), masterStatus: s(r.master_status) })),
    engineerWork: map(work, (r) => (s(r.id) ? {
    ...
    mixVersions: map(versions, (r) => (s(r.id) ? { id: String(r.id), workId: s(r.sound_engineer_work_id), status: s(r.status), createdAt: s(r.created_at) } : null)),
    mixComments: map(comments, (r) => ({ versionId: s(r.mix_version_id), status: s(r.status) })),
    finalFiles: map(finals, (r) => ({ workId: s(r.work_id), createdAt: s(r.created_at) })),
    deliveries: map(deliv, (r) => {
