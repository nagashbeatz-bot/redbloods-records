...
export function useRole(): ClientRole {
  ...
  useIsoLayoutEffect(() => {
    try {
      ...
      if (cached === "owner" || cached === "victor" || cached === "steven" || cached === "shalev" || cached === "cleantone") setRole(cached);
    } catch { /* ignore */ }
  }, []);
  ...
  useEffect(() => {
    ...
    fetch("/api/me")
      ...
      .then((d) => {
        if (!alive) return;
        if (d?.role === "owner" || d?.role === "victor" || d?.role === "steven" || d?.role === "shalev" || d?.role === "cleantone") {
          setRole(d.role);
